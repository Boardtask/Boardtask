    ...
    E: SqliteExecutor<'e>,
{
    sqlx::query_as::<_, OrgMemberWithEmail>(
        "SELECT om.user_id, u.email, om.role, COALESCE(u.first_name, '') AS first_name, COALESCE(u.last_name, '') AS last_name, u.profile_image_url FROM organization_members om JOIN users u ON u.id = om.user_id WHERE om.organization_id = ? ORDER BY om.role, u.email",
    )
    .bind(organization_id.as_str())
    .fetch_all(executor)
    .await
